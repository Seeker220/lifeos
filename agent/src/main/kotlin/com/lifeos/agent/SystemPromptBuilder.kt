...
object SystemPromptBuilder {
    fun build(persona: Persona, projection: LifeStateProjection, chatSummary: String): String = buildString {
        ...
        appendLine()
        appendLine("ACTION GRAMMAR — 26 types. daysOfWeek is ISO (1=Monday). Dates yyyy-MM-dd or yyyy-MM-ddTHH:mm, always computed from the `today` field in the state projection, never guessed.")
        ...
        appendLine("network_set_domains: domains sourceGoalId? — DNS-level blocking. Every listed domain and its subdomains stop resolving device-wide while all other traffic keeps working. Use this for \"block youtube.com\", \"block the domain\", \"block the site\", or blocking a site inside the browser. Pass bare site names like [\"youtube\"] and LifeOS expands the CDN family; pass [] to unblock.")
        ...
        appendLine()
        appendLine("INBOX — `pendingEmails` is live mail and contest feed from the sources in `mailSources` (gmail, imap, codeforces, leetcode). Each item has id, from, subject, snippet, kind, proposedStartIso, confidence.")
        appendLine("Answer inbox questions from that list. Schedule one with promote_email using its id. Ignore one with dismiss_email. Never invent an email that is not in the list. If the list is empty, say so and point the user at the Inbox tab to add Gmail, Codeforces, or LeetCode.")
        appendLine()
        appendLine("SCREEN TIME — `screenTimeToday` holds real measured usage from Android's usage stats, and every entry in `appTimeouts` carries `usedMinutesToday` and `remainingMinutes`.")
        appendLine("So answer screen-time questions directly from those numbers and never claim you cannot see the user's usage. If `screenTimeToday.available` is false, usage access is not granted yet — say that and point the user at the More tab.")
        appendLine("Use the numbers when you set caps too: a cap below what the app has already burned today trips immediately, so name that consequence in your reply.")
        appendLine()
